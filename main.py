...
def build_usg_template(customer_ipv4_wan_addr_str, customer_ipv4_lan_addr_mask_str, isp_ipv6_prefix_str, isp_ipv4_gateway_addr):
    """
    ...
    return {
        "interfaces": {
            ...
            "tunnel": {
                "tun0": {
                    "6rd-default-gw": "::" + isp_ipv4_gateway_addr,
...
def config():
    response = make_response(jsonify(build_usg_template(
        request.args.get('customer_ipv4_wan_addr', default=detect_ipaddr()),
        request.args.get('customer_ipv4_lan_addr_mask'),
        request.args.get('isp_ipv6_prefix'),
        request.args.get('isp_ipv4_gateway_addr'),
    )))
...
@app.route('/ipaddr')
def ipaddr():
    return jsonify({"ipaddr": detect_ipaddr()})
